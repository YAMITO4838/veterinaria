...
public class RazaApi {
    ...
    @GetMapping("/listar")
    public ResponseEntity<List<Raza>> getAll(@RequestParam(required = false) String title) {
        try {
            List<Raza> razas = facade.listar(TipoClase.RAZA);
            if (razas.isEmpty()) {
    ...
    @PostMapping
    public ResponseEntity<Raza> create(@RequestBody Raza entidad) {
        try {
            Raza _entidad = facade.crear(entidad, TipoClase.RAZA);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Raza> update(@PathVariable("id") Integer id, @RequestBody Raza entidad)
            throws NoSuchFieldException {
        try {
            if (entidad != null) {
                facade.actualizar(entidad, TipoClase.RAZA);
    ...
    public ResponseEntity<HttpStatus> statusServicio(@PathVariable Integer id, @RequestBody Estado estado) {
        try {
            facade.cambiarEstado(id, estado, TipoClase.RAZA);
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
