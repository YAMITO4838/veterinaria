...
public class ProductoApi {
    ...
    @GetMapping("/listar")
    public ResponseEntity<List<Producto>> getAll(@RequestParam(required = false) String title) {
        try {
            List<Producto> productos = facade.listar(TipoClase.PRODUCTO);
    ...
    public ResponseEntity<Producto> create(@RequestBody Producto entidad) {
        try {
            Producto _entidad = facade.crear(entidad, TipoClase.PRODUCTO);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Producto> update(@PathVariable("id") Integer id, @RequestBody Producto entidad)
            throws NoSuchFieldException {
        try {
            if (entidad != null) {
                facade.actualizar(entidad, TipoClase.PRODUCTO);
    ...
    public ResponseEntity<HttpStatus> statusServicio(@PathVariable Integer id, @RequestBody Estado estado) {
        try {
            facade.cambiarEstado(id, estado, TipoClase.PRODUCTO);
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
